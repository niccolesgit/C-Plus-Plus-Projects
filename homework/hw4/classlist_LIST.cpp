...
void enroll_student(const string& id_number, unsigned int max_students, 
                    list<string>& enrolled, list<string>& waiting);
...
int main() {
  ...
  cout << "\nAt the end of the enrollment period, the following students\n"
       << "are in the class:\n\n";
  for (list<string>::iterator i=enrolled.begin(); i != enrolled.end(); i++) {
    cout << *i << endl;
  }
  if (!waiting.empty()) {
    cout << "\nStudents are on the waiting list in the following order:\n";
    for (list<string>::iterator j=waiting.begin(); j != waiting.end(); j++) {
      cout << *j << endl;
    }
  }
  return 0;
...
void remove_student(const string& id_number, unsigned int max_students, 
  ...
  bool found = false;
  list<string>::iterator itr = enrolled.begin();
  ...
  if (found) {
    ...
    found = false;
    list<string>::iterator wait_itr = waiting.begin();
    while (!found && wait_itr != waiting.end()) {
      found = *wait_itr == id_number;
      if (!found) ++wait_itr;
    }
    if (found) {
      waiting.erase(wait_itr);
