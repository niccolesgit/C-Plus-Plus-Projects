...
void StudentTests(){
	 ...
	 */
	//checks all concat tests and that they can be printed
	
	Rope root_left(ConcatTestRopeLeft());
	std::cout << "***Concated left" << std::endl;
	Rope root_right(ConcatTestRopeRight());
	std::cout << "***Concated right" << std::endl;
	Rope root_index(IndexTestRope());
	std::cout << "***Concated index" << std::endl;

	std::cout << "***Printing left"<< std::endl;
	root_left.print(std::cout);
	root_left.print_strings(std::cout);
	std::cout << "***Printing right"<< std::endl;
	root_right.print(std::cout);
	root_right.print_strings(std::cout);
	std::cout << "***Printing index"<< std::endl;
	root_index.print(std::cout);
	root_index.print_strings(std::cout);
	std::cout << "\n";

	//checks that it can concat
	//and prints concated right and left
	root_left.concat(root_right);
	std::cout << "***Concated right to the left" << std::endl;
	std::cout << "***Printing left" << std::endl;
	root_left.print(std::cout);
	root_left.print_strings(std::cout);
	std::cout << "***Printing right" << std::endl;
	root_right.print(std::cout);
	root_right.print_strings(std::cout);
	//testing if the rope matches the string length
	//length is longer than my name is simon
	std::string test = "My name is Jessica";
	//checks if the size is the not the same as the length
	if(root_index.size() != (int)test.length()){
		//if it does it passes
		std::cout<<"PASSED: Rope does not match string length"<<std::endl;
	}else{
		//else it fails
		std::cout<<"FAILED: Rope matches string length"<<std::endl;
	}
	//tests if the indexing works (checks if there is an error in the string)
	std::string test2 = "Hello my name is Simin"; //Simin instead of Simon
	//char to send it
	char c;
	//loops through the string
	for(unsigned int i=0; i<test2.size(); i++){
		//checks the index of the root with the string
		if(!root_index.index(i,c) || c != test2[i]){
			std::cout<< "PASSED: Wrong character(s) in string"<<std::endl;

		}
	}
	//tests any out of bounds errors. 
	std::string test3 = "Hello my name is Simon";
	std::string var;
	int length = test3.length();
	if(!root_index.report(length+1, length,var)){
		std::cout<<"PASSED: Unable to compute report"<<std::endl;
	}
