...
public:
	...
	rope_iterator operator++(icopynt);
	int weight() const;
...
public:
	...
	void deleterope(Node* root); //recursive function used to delete the whole rope
	...
	void update(Node* top);
	void leaf_weight(Node* top);
	void addsize(int x) {size_ += x;}
...
bool is_leaf(Node* p);
Node* copy(Node* root);
Node* concat_no_copy(Node* X1, Node* X2); //adds other string to me not by copy
