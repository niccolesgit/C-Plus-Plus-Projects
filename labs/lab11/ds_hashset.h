...
public:
  ...
  std::pair< iterator, bool > insert(KeyType const& key) {
    ...
    while(itr != m_table[index].end()){
      if(*itr == key){
        return std::make_pair(iterator(this, index, itr), false);
      }
      itr++;
    }

    m_table[index].push_front(key);
    return std::make_pair(iterator(this, index, m_table[index].begin()), true);
  }
  ...
  iterator begin() {
    for(unsigned int i = 0; i < m_table.size(); ++i){
      if(!m_table[i].empty()) return iterator(this, i, m_table[i].begin());
    }
...
private:
  ...
  void resize_table(unsigned int new_size) {
    ...
    std::vector< std::list<KeyType> > n_table(new_size);
    ...
    while(itr != this->end()){
      std::string key = *itr;
      unsigned int hash_number = m_hash(key);
      unsigned int index = hash_number % new_size;
      hash_list_itr itr = n_table[index].begin();
      m_table[index].push_front(key);
      itr++;
    }
