...
public:
  ...
  // FIND, INSERT & ERASE
  iterator find(const T& key_value) { return /*find(key_value, root_)*/my_find(key_value, root_); }
  ...
  T accumulate(T sum){
    if(root_){
      sum += accum_helper(root_);
    }
    return sum;
  }

  T accum_helper(TreeNode<T>* p){
    T sum = p->value;
    if(p->left){
      sum += accum_helper(p->left);
    }
    if(p->right){
      sum += accum_helper(p->right);
    }
    return sum;
  }
...
private:
  ...
  TreeNode<T>*  copy_tree(TreeNode<T>* old_root) {
    ...
    if(old_root == NULL){
      ...
      TreeNode<T>* newR = new TreeNode<T>;
      newR->value = old_root->value;
      newR->left = copy_tree(old_root->left);
      newR->right = copy_tree(old_root->right);
      root_ = newR;
    }
    return root_;
  }
  ...
  iterator my_find(const T& key_value, TreeNode<T>* p){
    while(p){
      if(p->value > key_value){
        p= p->left;
      }else if(p->value < key_value){
