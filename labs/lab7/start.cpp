...
int Blocked(int x, int y, std::vector<std::vector<GRID_STATUS> >& blocked, std::vector<int> & coords){
 int c = 0;
    if(x == 0 && y ==0){
        return 1;
    }
    if(x > 0 && blocked[x-1][y] != GRID_BLOCKED){
      coords.push_back(x);
      coords.push_back(y);
        c+=Blocked(x-1, y, blocked);
    }
    if(y > 0 && blocked[x][y-1]!= GRID_BLOCKED){
      coords.push_back(x);
      coords.push_back(y);
        c+=Blocked(x, y-1, blocked);
    }
    return c;
    }
...
int main(int argc, char* argv[]) {
  ...
  std::cout << "Paths when blocked: " << Blocked(start_x, start_y, blocked_grid, coordl) << std::endl;
